...
// const cleared =
//     'A=’’,B=!A+A,C=!B+A,D=A+{},E=B[A++],F=B[G=A],H=++G+A,I=D[G+H],B[I+=D[A]+(B.C+D)[A]+C[H]+E+F+B[G]+I+E+D[A]+F][I](’p’+F+D[A]+’m’+’p’+E+’(A,++A)’)()'.replace(
//         /’/g,
//         "'",
//     );
// console.log(`cleared message: ${cleared}`);

const aurebesh_input = JSON.parse(`{\"message\":\"${cleared}\"}`);
